...
class NeonovaSnapshotController {
    ...
    static async buildData(username, friendlyName, startDate, endDate) {
        try {
            ...
            // Pagination can return null when the range has no logs at all.
            // Treat as empty rather than a fetch failure — empty range is a
            // valid result, especially when drilling into a quiet day.
            const safeRaw = Array.isArray(raw) ? raw : [];

            const cleanResult = NeonovaCollector.cleanEntries(safeRaw);
            const cleaned = Array.isArray(cleanResult)
                ? cleanResult
                : (cleanResult?.cleanedEntries || []);

            // computeMetrics needs entries to derive a state, but with an
            // empty range we just synthesize a minimal metrics object so the
            // chart can render an empty timeline honestly.
            let metrics = NeonovaAnalyzer.computeMetrics(cleanResult, startDate, endDate);
            if (!metrics) {
                metrics = {
                    percentConnected: 0,
                    longDisconnects: [],
